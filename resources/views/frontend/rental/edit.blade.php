    ...
    <div class="bg-brand-bg min-h-screen py-8">
        <div class="max-w-4xl mx-auto px-4">
            <div class="flex items-center justify-between mb-6">
                <h1 class="text-2xl font-bold text-brand-dark">Edit Vehicle</h1>
            ...
            <div class="bg-white rounded-xl shadow-md p-6">
                <form action="{{ route('admin.rental.vehicles.update', $vehicle) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                        ...
                        <div>
                            ...
                            <input type="number" name="number_of_seats" value="{{ old('number_of_seats', $vehicle->number_of_seats) }}" min="1"
                        ...
                        <div>
                            ...
                            <select name="condition"
                                ...
                                @foreach(['new' => 'New', 'good' => 'Good', 'average' => 'Average', 'old' => 'Old'] as $val => $label)
                                    <option value="{{ $val }}" {{ old('condition', $vehicle->condition) === $val ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @error('condition')
                                <p class="text-brand-maroon text-xs mt-1">{{ $message }}</p>
                                ...
                                @foreach(['boot' => 'Boot Storage', 'head' => 'Head Storage', 'both' => 'Both', 'neither' => 'Neither'] as $val => $label)
                                    <label class="flex items-center gap-2 text-sm cursor-pointer">
                                        <input type="radio" name="luggage_storage" value="{{ $val }}"
                                               {{ old('luggage_storage', $vehicle->luggage_storage) === $val ? 'checked' : '' }}
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                        ...
                        {{-- Main Image --}}
                        <div>
                            <label class="block text-sm font-medium text-brand-dark mb-1">Main Image</label>
                            <div class="mb-2">
                        ...
                        {{-- Additional Images --}}
                        <div>
                            <label class="block text-sm font-medium text-brand-dark mb-1">Additional Images <span class="text-gray-400 font-normal">(max 3)</span></label>
                            @if($vehicle->images->isNotEmpty())
                                ...
                                <p class="text-xs text-gray-400 mb-2">Uploading new images will replace all existing additional images</p>
                            @endif
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6 mt-4">
                        <div>
                            <label class="block text-sm font-medium text-brand-dark mb-1">Fare Per Day (NPR) <span class="text-brand-maroon">*</span></label>
                            <input type="number" name="fare_per_day" value="{{ old('fare_per_day', $vehicle->fare_per_day) }}" step="0.01" min="0"
                            ...
                            <p class="text-xs text-gray-400 mt-1">Displayed on vehicle listing and detail pages.</p>
                            @error('fare_per_day')
                                <p class="text-brand-maroon text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6 mt-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="oil_price" value="{{ old('oil_price', $vehicle->oil_price) }}" step="0.01" min="0"
                        ...
                        <div>
                            ...
                            <input type="number" name="driver_allowance" value="{{ old('driver_allowance', $vehicle->driver_allowance) }}" step="0.01" min="0"
                        ...
                        <div>
                            ...
                            <input type="number" name="profit_margin" value="{{ old('profit_margin', $vehicle->profit_margin) }}" step="0.01" min="0"
